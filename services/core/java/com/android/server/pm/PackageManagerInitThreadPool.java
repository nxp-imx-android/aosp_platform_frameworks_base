 ...
 * @hide
 */
public class PackageManagerInitThreadPool {
    private static final String TAG = PackageManagerInitThreadPool.class.getSimpleName();
    private static final int SHUTDOWN_TIMEOUT_MILLIS = 20000;
    private static final boolean IS_DEBUGGABLE = Build.IS_DEBUGGABLE;

    private static PackageManagerInitThreadPool sInstance;

    private ExecutorService mService = ConcurrentUtils.newFixedThreadPool(
            9,
            "package-manager-init-thread", Process.THREAD_PRIORITY_FOREGROUND);

    private List<String> mPendingTasks = new ArrayList<>();

    public static synchronized PackageManagerInitThreadPool get() {
        if (sInstance == null) {
            sInstance = new PackageManagerInitThreadPool();
        }
